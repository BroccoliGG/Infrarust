...
| `version_protocol` | integer | Protocol number reported to the client. A value that differs from the client's protocol makes the client show `version_name` as an incompatible version. |
